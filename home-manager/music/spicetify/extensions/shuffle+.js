...
(async function shufflePlus() {
  ...
  function getConfig() {
    try {
      const parsed = JSON.parse(
        Spicetify.LocalStorage.get("shufflePlus:settings"),
      );
  ...
  new Spicetify.Menu.Item("Shuffle+", false, settingsPage, "shuffle")
    .register();
  ...
  function shouldAddShufflePlusLocal(uri) {
    const uriObj = Spicetify.URI.fromString(uri[0]);
    if (
      Spicetify.Platform.History.location.pathname === "/collection/local-files"
    ) {
      switch (uriObj.type) {
        case Type.TRACK:
  ...
  async function fetchPlaylistTracks(uri) {
    const res = await Spicetify.CosmosAsync.get(
      `sp://core-playlist/v1/playlist/spotify:playlist:${uri}/rows`,
      {
        policy: { link: true, playable: true },
      },
    );
    return res.rows.filter((track) => track.playable).map((track) =>
      track.link
    );
  }
  ...
  async function fetchFolderTracks(uri) {
    ...
    async function fetchNested(folder) {
      ...
      for (const i of folder.rows) {
        if (i.type === "playlist") {
          requestPlaylists.push(
            await fetchPlaylistTracks(i.link.split(":")[2]),
          );
  ...
  async function scanForTracksFromAlbums(res, artistName, type) {
    ...
    for (const album of res) {
      ...
      Spicetify.showNotification(
        `${artistFetchTypeCount[type]} / ${res.length} ${type}s`,
      );

      for (const track of albumRes) {
        if (
          !CONFIG.artistNameMust || track.artists.items.some((artist) =>
  ...
  async function fetchArtistTracks(uri) {
    ...
    const discography = await Spicetify.GraphQL.Request(
    ...
    const artistSingles = releases.filter((album) =>
      album.type === "SINGLE" || album.type === "EP"
    );
    ...
    const allArtistAlbumsTracks = CONFIG.artistMode !== "single"
      ? await scanForTracksFromAlbums(artistAlbums, artistName, "album")
      : [];
    const allArtistSinglesTracks = CONFIG.artistMode !== "album"
      ? await scanForTracksFromAlbums(artistSingles, artistName, "single")
      : [];
  ...
  async function fetchArtistTopTenTracks(uri) {
    ...
    if (errors) throw errors[0].message;
    return data.artistUnion.discography.topTracks.items.map(({ track }) =>
      track.uri
    );
  }
  ...
  async function fetchShows(uri) {
    const res = await Spicetify.CosmosAsync.get(
      `sp://core-show/v1/shows/${uri}?responseFormat=protobufJson`,
    );
    return res.items.filter((track) => track.episodePlayState.isPlayable).map((
      track,
    ) => track.episodeMetadata.link);
  }
